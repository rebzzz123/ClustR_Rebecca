...
clustr <- function(id=NULL, status=NULL, date=NULL, lat=NULL, long=NULL, time_incs=c(90,180,365), space_incs=c(.025, .05, .1, .5), n=NULL, bootstraps=5, data){
  ...
  for(c in 1:bootstraps){
    ...
    # creating_new_column_names
    new_col_names <- function(g,t) {
      ...
      return(radius_time_cols)
    }
    
    #create all combinations of radius & time
    radius_time_combo <- (expand.grid(radius_incs,time_incs))
    
    #call new_col_names function to create new column names
    new_cols <- as.vector(mapply(new_col_names,radius_time_combo[,1],radius_time_combo[,2]))
    new_cluster_col_names <- append(cluster_col_names, new_cols)
    colnames(cluster) <- new_cluster_col_names
    ...
    x <- foreach(a=1:samples) %do% clustr_null()
    sample_result <- as.data.frame(list.rbind(x))
